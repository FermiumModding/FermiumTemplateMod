package examplemod.mixin.examplemod;
...
import examplemod.mixintarget.ExampleMixinTarget;
...
//Mixin Classes don't need to be abstract, but its often useful and there's also no need to make them not abstract.
    ...
    private static String examplemod_modifyConstant(String original) {
    ...
    @ModifyArg(
            ...
            at = @At(
                    value = "INVOKE",
                    target = "Lexamplemod/mixintarget/ExampleMixinTarget;callToModifyArg(III)V"
            ),
            index = 1 // middle parameter
    )
    private static int examplemod_modifyArg(int j) {
        return j;
    }
    ...
    @ModifyExpressionValue(
            method = "targetStaticMethod",
            at = @At(value = "FIELD", target = "Lexamplemod/mixintarget/ExampleMixinTarget;staticFieldToShadow:Ljava/lang/String;")
    )
    private static String examplemod_modifyExpressionValue(String original) {
        return original; //modify here
    }
    ...
    private static int examplemod_modifyVariable(int localVar) {
    ...
    private static float examplemod_modifyReturnValue(float original) {
        return original; // Modify here
    }
    ...
    @WrapWithCondition(
            ...
            at = @At(value = "INVOKE", target = "Lexamplemod/mixintarget/ExampleMixinTarget;callToWrapWithCondition(II)V")
    )
    private boolean examplemod_wrapWithCondition(ExampleMixinTarget instance, int x, int y) {
    ...
    @WrapOperation(
            method = "targetExampleMethod",
            at = @At(value = "INVOKE", target = "Lexamplemod/mixintarget/ExampleMixinTarget;callToWrapOperation(Ljava/lang/String;Z)Ljava/lang/String;")
    )
    private String examplemod_wrapOperation(ExampleMixinTarget instance, String a, boolean b, Operation<String> original) {
        //This would normally be an @Inject
        ExampleMod.LOGGER.info("@WrapOperation: Before helperMethod()");
        ...
        ExampleMod.LOGGER.info("@WrapOperation: After helperMethod()");
    ...
    private int examplemod_redirect(String instance) {
